...
import "../registry/SNSRegistry.sol";
...
/**
 * @dev An SNS registrar that allows the owner of a SNS name to claim the
 *      corresponding name in SNS.
 */
...
contract DNSRegistrar is IDNSRegistrar, IERC165 {
    ...
    /**
     * @dev This contract's owner-only functions can be invoked by the owner of the SNS root.
     */
    modifier onlyOwner() {
        Root root = Root(sns.owner(bytes32(0)));
    ...
    function proveAndClaim(
        ...
        sns.setSubnodeOwner(rootNode, labelHash, addr);
    }
    ...
    function proveAndClaimWithResolver(
        ...
        if (addr != address(0)) {
            ...
            sns.setSubnodeRecord(rootNode, labelHash, owner, resolver, 0);
        }
    }
        ...
        returns (bytes32 node)
    {
        ...
        if (owner != address(this)) {
            if (parentNode == bytes32(0)) {
                ...
                sns.setSubnodeOwner(parentNode, label, address(this));
            }
        }
        return node;
    }
