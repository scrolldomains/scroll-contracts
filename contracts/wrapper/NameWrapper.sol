...
import {SNS} from "../registry/SNS.sol";
    ...
    ERC20Recoverable
{
    ...
    SNS public immutable override sns;
     ...
     * @param id Label as a string of the .scroll domain to wrap
        ...
        onlyOwner
    {
        if (address(upgradeContract) != address(0)) {
            ...
            sns.setApprovalForAll(address(upgradeContract), false);
        }
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            sns.setApprovalForAll(address(upgradeContract), true);
        }
    }
     ...
     * @dev Can be called by the owner of the name on the .scroll registrar or an authorised caller on the registrar
    ...
    function wrapETH2LD(
        ...
        // transfer the sns record back to the new owner (this contract)
     ...
     * @param resolver The resolver address to set on the SNS registry (optional).
     * @param fuses Initial fuses to set
     * @param expiry When the fuses will expire
     * @return registrarExpiry The expiry date of the new name on the .scroll registrar, in seconds since the Unix epoch.
     */
    ...
    /**
     * @notice Renews a .scroll second-level domain.
     * @dev Only callable by authorised controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.scroll').
     * @param duration The number of seconds to renew the name for.
     * @return expires The expiry date of the name on the .scroll registrar, in seconds since the Unix epoch.
     */
     ...
     * @param name The name to wrap, in SNS format
     * @param wrappedOwner Owner of the name in this contract
    ...
    function wrap(
        ...
        if (owner != msg.sender && !sns.isApprovedForAll(owner, msg.sender)) {
            revert Unauthorised(node, msg.sender);
        }
     ...
     * @param labelhash Labelhash of the .scroll domain
     * @param registrant Sets the owner in the .scroll registrar to this address
    ...
    /**
     * @notice Upgrades a .scroll wrapped domain by calling the wrapETH2LD function of the upgradeContract
     *     and burning the token of this contract
     * @dev Can be called by the owner of the name in this contract
     * @param label Label as a string of the .scroll name to upgrade
     * @param wrappedOwner The owner of the wrapped name
     */
    ...
    /**
     * @notice Upgrades a non .scroll domain of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    /** 
    /* @notice Sets fuses of a name that you own the parent of. Can also be called by the owner of a .scroll name
     * @param parentNode Parent namehash of the name e.g. vitalik.xyz would be namehash('xyz')
     * @param labelhash Labelhash of the name, e.g. vitalik.xyz would be keccak256('vitalik')
        ...
        returns (bytes32 node)
    {
        ...
        if (!isWrapped(node)) {
            sns.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    /**
     * @notice Sets records for the name in the SNS Registry
    ...
    {
        sns.setRecord(node, address(this), resolver, ttl);
    ...
    modifier canCallSetSubnodeOwner(bytes32 node, bytes32 labelhash) {
        ...
        address owner = sns.owner(subnode);
    ...
    function onERC721Received(
        ...
        //check if it's the sns registrar ERC721
    ...
    function _normaliseExpiry(
        ...
        // .scroll names will check registrar, non .scroll check parent
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
