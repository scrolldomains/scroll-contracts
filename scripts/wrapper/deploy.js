...
const snsAppSourcePath = '../sns-app/cypress.env.json'
...
async function main() {
  ...
  ({
    ...
    METADATA_HOST:metadataHost = 'sns-metadata-service.appspot.com'
  } = parsedFile)
  if(network.name === 'localhost'){
    const addresses = JSON.parse(fs.readFileSync(snsAppSourcePath, 'utf8'))
    registryAddress = addresses.snsAddress
    registrarAddress = addresses.baseRegistrarAddress
    metadataUrl = 'http://localhost:8080/name/0x{id}'
    if(!(addresses.snsAddress && addresses.baseRegistrarAddress)){
      throw('please run yarn preTest on ../sns-app')
    }  
  }else{
